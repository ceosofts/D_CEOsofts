...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create('employees', function (Blueprint $table) {
            ...
            $table->integer('age')->nullable(); // ✅ อายุ (คำนวณอัตโนมัติ)
            ...
            $table->unsignedBigInteger('department_id'); // แผนก
            $table->foreign('department_id')->references('id')->on('departments')->onDelete('cascade');
            
            // ✅ เพิ่มคอลัมน์ใหม่
            $table->unsignedBigInteger('position_id')->nullable(); // ตำแหน่ง
            $table->foreign('position_id')->references('id')->on('positions')->onDelete('set null');
            $table->decimal('salary', 10, 2)->nullable(); // เงินเดือน
            $table->enum('employment_status', ['active', 'resigned', 'terminated', 'on_leave'])->default('active'); // สถานะการทำงาน
